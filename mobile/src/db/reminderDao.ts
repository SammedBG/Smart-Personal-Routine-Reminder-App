...
export async function loadAllRemindersFromDb(): Promise<Reminder[]> {
  ...
  const rawRows: any[] = typeof db.getAllAsync === 'function'
    ? await db.getAllAsync('SELECT * FROM reminders WHERE deleted = 0')
    : (await db.executeSql('SELECT * FROM reminders WHERE deleted = 0'))[0].rows._array || [];
